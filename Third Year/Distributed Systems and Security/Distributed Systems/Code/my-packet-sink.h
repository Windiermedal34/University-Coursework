...
class MyPacketSink : public Application
{
  public:
    ...
    void SetServerTime(const double& m_enqueueTime, const double& m_dequeueTime);
    double GetServerTime() const;

    void SetHighServer(const double& m_highEnqueue, const double& m_highDequeue);
    double GetHighServer() const;

    void SetEnqueueTime(const double& time);
    void SetHighEnqueue(const double& time);
    void SetDequeueTime(const double& time);
    void SetHighDequeue(const double& time);

    double GetEnqueueTime() const;
    double GetHighEnqueue() const;
    double GetDequeueTime() const;
    double GetHighDequeue() const;
    uint64_t GetCounter() const;

    std::list<double> GetEnqueueTimeList() const;
    std::list<double> GetDequeueTimeList() const;
    std::list<double> GetHighEnqueueList() const;
    std::list<double> GetHighDequeueList() const;
    std::list<double> GetAllServerTimes() const;

    uint64_t GetLogicalTime() const;
  ...
  private:
    ...
    MyQueue<Packet> queue;
    // MyQueue<Packet> highQueue;
    uint64_t counter;

    double m_enqueueTime;
    double m_highEnqueue;
    double m_dequeueTime;
    double m_highDequeue;
    double highServer;
    double serverTime;
    std::list<double> enqueueTimes;
    std::list<double> highEnqueueList;
    std::list<double> dequeueTimes;
    std::list<double> highDequeueList;
    std::list<double> allServerTimes;

    uint64_t m_logicalTime;
